...
# never typed into the query), ctrl-p peeks at the highlighted entry in a
# nested client (alt-1..9 peek the Nth entry; terminals can't send
# ctrl+digit), ctrl-x kills a session.

source "$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)/helpers.sh"

require_cmd fzf "install it (brew install fzf) to use the project picker"

# Plain digit = open Nth entry; alt+digit = peek Nth entry. peek.sh uses
# execute (not execute-silent) because the nested tmux client needs the tty.
digit_binds=()
for i in 1 2 3 4 5 6 7 8 9; do
  digit_binds+=(--bind "$i:pos($i)+accept")
  digit_binds+=(--bind "alt-$i:pos($i)+execute('$COCKPIT_SCRIPTS/peek.sh' {1})")
done
...
selection="$("$COCKPIT_SCRIPTS/project-list.sh" | fzf \
  ...
  --layout=reverse \
  --prompt='project ❯ ' \
  --header=$'enter/1-9: open · ctrl-p/alt-1-9: peek (prefix+d returns)\nctrl-x: kill session · esc: cancel' \
  --bind 'left-click:accept' \
  "${digit_binds[@]}" \
  --bind "ctrl-p:execute('$COCKPIT_SCRIPTS/peek.sh' {1})" \
